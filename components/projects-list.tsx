    ...
    <div className="mx-auto max-w-6xl px-6 py-8 md:px-8">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <IconNotebook className="mb-3 size-10 text-muted-foreground/30" />
          <p className="text-sm text-muted-foreground">
            {search.trim() ? "没有找到匹配的笔记" : "还没有笔记，点击右上角按钮开始创建"}
          </p>
        </div>
      ) : viewMode === "grid" ? (
        /* Grid view */
            ...
            >
              ...
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-sm font-medium">
                  <IconNotebook className="size-4 shrink-0 text-muted-foreground" />
                  <span className="truncate">{project.name}</span>
                </CardTitle>
                <CardDescription className="flex items-center gap-1 text-xs">
                  <IconFileText className="size-3" />
                  {project.fileCount} 篇文档
                </CardDescription>
              </CardHeader>
              <CardContent className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="relative z-20 rounded-none p-1.5 text-muted-foreground opacity-0 transition-opacity hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100"
                  title="删除"
                >
                  {deleting === project.id ? (
                    <IconLoader2 className="size-4 animate-spin" />
        ...
        <div className="divide-y divide-border rounded-none border">
          {filteredProjects.map((project) => (
            <div
              key={project.id}
              className="group relative flex items-center gap-4 px-4 py-3 transition-colors hover:bg-muted/50"
            >
              <Link
                href={`/docs/projects/${encodeURIComponent(project.id)}`}
                className="absolute inset-0 z-10"
