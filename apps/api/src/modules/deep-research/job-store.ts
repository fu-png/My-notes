 ...
 * 客户端通过 SSE 端点订阅进度更新
 *
 * 迁移自 apps/web/lib/deep-research/job-store.ts
 * 改动：ResearchJob 新增 userId 字段，createJob 新增 userId 参数，
 * 用于 subscribe 路由的归属校验
 */
...
export function createJob(id: string, projectId: string, query: string, userId: string): ResearchJob {
