...
export const ChatPanel = React.memo(function ChatPanel({
  ...
  if (!showAI && !agentMode) return null

  return (
    <div
      ref={aiPanelRef}
      className={agentMode ? "relative flex flex-1 flex-col overflow-hidden bg-background" : "relative hidden shrink flex-col overflow-hidden border-l bg-background md:flex"}
      style={agentMode ? undefined : { width: aiPanelWidth, minWidth: 200 }}
    >
      {/* Resize handle */}
      <div
      ...
      />
      {/* Chat header */}
      <div className="flex h-[49px] shrink-0 items-center justify-between border-b px-3">
        <div className="flex min-w-0 items-center gap-2">
          {showHistory ? (
            ...
            <>
              ...
              <span className={`size-1.5 shrink-0 rounded-full ${aiConfigured ? "bg-green-500" : "bg-muted-foreground/40"}`} title={aiConfigured ? "已配置" : "未配置 API Key"} />
            </>
          ) : (
            <>
        ...
        <div className="flex items-center gap-0.5">
          ...
          {agentMode && onToggleFileList && (
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon-sm" onClick={onToggleFileList} aria-label={fileListCollapsed ? "展开文件列表" : "收起文件列表"}>
                  {fileListCollapsed ? <IconLayoutSidebarRightExpand className="size-4" /> : <IconLayoutSidebarRightCollapse className="size-4" />}
                </Button>
              </TooltipTrigger>
              <TooltipContent side="left">{fileListCollapsed ? "展开文件列表" : "收起文件列表"}</TooltipContent>
            </Tooltip>
          )}
        </div>
      </div>
...
/>
      ...
      <div ref={chatScrollRef} onScroll={onChatScroll} className="min-h-0 flex-1 overflow-y-auto">
        <div className={`mx-auto w-full max-w-3xl p-4 ${chatMessages.length <= 1 && !chatLoading ? "flex h-full flex-col" : ""}`}>
...
<PptFlowControls
  ...
  pptSession={pptSession}
      ...
      <div className="shrink-0 border-t px-3 py-2.5">
        <div className="mx-auto w-full max-w-3xl border border-border bg-background px-3 py-2">
          <textarea
            value={chatInput}
      ...
      <div className="border-b px-3 pb-2 pt-3">
        <div className="relative">
          ...
          <input
            ...
            aria-label="搜索历史对话"
            className="h-8 w-full rounded-md border border-input bg-background pl-8 pr-8 text-sm outline-none ring-offset-background placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
          />
          {searchQuery && (
            <button
